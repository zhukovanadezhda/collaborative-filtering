...
class ALS:
    ...
    def fit(self, R: np.ndarray) -> ALS:
        ...
        self.user_factors = np.random.normal(scale=0.1, size=(m, self.n_factors))
        self.item_factors = np.random.normal(scale=0.1, size=(n, self.n_factors))
        ...
        logger.info(
            ...
            "factors=%d, iters=%d",
            m, n, self.n_factors, self.n_iters,
        )
...
def complete_ratings(
    ...
    model = ALS(
        n_factors=int(params.get("n_factors", 100)),
        n_iters=int(params.get("n_iters", 40)),
        reg=float(params.get("reg", 0.8)),
